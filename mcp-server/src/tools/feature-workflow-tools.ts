...
export class FeatureWorkflowTools {
  ...
  getTools(): Tool[] {
    return [
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            completed: { type: "boolean", description: "Optional completion filter" }
          },
          required: ["workflow_id"]
        }
      },
      ...
      },
      {
        ...
        description: 'Update a task status with markdown and database synchronization',
        inputSchema: {
          type: "object",
          properties: {
            ...
            notes: { type: "string", description: "Optional notes about the task update" }
          },
      ...
      },
      {
        ...
        description: 'Add a new task with markdown and database synchronization',
        inputSchema: {
          type: "object",
          properties: {
            workflow_id: { type: "string", description: "The workflow ID" },
            phase: { type: "string", description: "The phase this task belongs to" },
            description: { type: "string", description: "Task description" },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            workflow_id: { type: "string", description: "The workflow ID" },
            phase: { type: "string", description: "The phase to create a checkpoint for" }
          },
      ...
      },
      {
        ...
        description: 'Create a feature directory structure with specs, tasks, and documentation',
        inputSchema: {
          type: "object",
          properties: {
            project_id: { type: "string", description: "The project ID" },
            feature_name: { type: "string", description: "The feature name (will be used as directory name)" },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            task_description: { type: "string", description: "The task description to find and update" },
            completed: { type: "boolean", description: "Whether the task is completed" },
            notes: { type: "string", description: "Optional notes about the task update" }
          },
          required: ["project_id", "feature_name", "task_description", "completed"]
        }
      },
      {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            project_id: { type: "string", description: "The project ID" },
            feature_name: { type: "string", description: "The feature name" },
            validation_notes: { type: "string", description: "Notes about feature validation and completion" }
          },
          required: ["project_id", "feature_name"]
        }
      }
  ...
  async listFeatureWorkflows(args: any) {
    try {
      ...
      const workflows = await this.manager.getWorkflowsByProject(data.project_id);
      return {
        success: true,
        workflows
      };
  ...
  private async handleGenerateProgressSummary(args: any) {
    try {
      const summary = await this.taskManager.generateProgressSummary(args.workflow_id);
  ...
  async createFeatureDirectory(args: any) {
    try {
      const { project_id, feature_name, description, tasks = [], workflow_type = 'feat' } = args;
  ...
  async updateFeatureTask(args: any) {
    try {
      const { project_id, feature_directory, task_description, completed, notes } = args;
      const fs = require('fs').promises;
      const path = require('path');
      
      const tasksFile = path.join(this.projectRoot, '.specs', project_id, feature_directory, 'tasks.md');
      ...
      const updatedContent = content.replace(
        new RegExp(`- \\[[ x]\\] ${task_description.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')}`, 'g'),
      ...
      try {
        ...
        for (const featureDir of featureDirectories) {
          ...
          if (stat.isDirectory() && /^\d+-/.test(featureDir)) {
            ...
            const progress = await this.getFeatureProgress({ project_id, feature_directory: featureDir });
  ...
  async createFeatureCheckpoint(args: any) {
    try {
      ...
      const progress = await this.getFeatureProgress({ project_id, feature_directory });
      
      if (!progress.success) {
        throw new Error(`Failed to get progress for feature '${feature_directory}'`);
      }
      ...
      const checkpointFile = path.join(this.projectRoot, '.specs', project_id, feature_directory, `checkpoint-${Date.now()}.md`);
...
export function createFeatureWorkflowTools(db: DatabaseManager, projectRoot: string) {
