...
export class FeatureWorkflowManager {
  ...
  private async createWorkflowDirectory(
    ...
    try {
      ...
      for (const template of templates) {
        ...
        try {
          ...
          const customizedContent = templateContent.replace(
            /\[Feature Name\]/g,
            workflow.feature_name
          );
  ...
  private async initializeWorkflowTasks(workflowId: string): Promise<void> {
    const defaultTasks = [
      ...
      {
        phase: 'user-stories',
        description: 'Define problem statement and business value',
      },
      {
        phase: 'user-stories',
        description: 'Write primary user stories with acceptance criteria',
      },
      {
        phase: 'user-stories',
        description: 'Define functional specifications',
      },
      {
        phase: 'user-stories',
        description:
          'Specify quality requirements (performance, security, reliability)',
      },
      {
        phase: 'user-stories',
      ...
      {
        phase: 'user-stories',
        description: 'Define success metrics and exclusions',
      },
      ...
      {
        phase: 'architecture',
        description: 'Define component architecture (frontend/backend)',
      },
      {
        phase: 'architecture',
        description: 'Design data architecture and database schema',
      },
      {
        phase: 'architecture',
        description: 'Specify API endpoints and data flow',
      },
      {
        phase: 'architecture',
        description: 'Define security and performance architecture',
      },
      {
        phase: 'architecture',
        description: 'Plan deployment and integration strategy',
      },
      ...
      {
        phase: 'implementation',
        description: 'Create frontend components and UI',
      },
      {
        phase: 'implementation',
      ...
      {
        phase: 'testing',
        description: 'Conduct performance and security testing',
      },
  ...
  async completeTask(taskId: string, notes?: string): Promise<void> {
    ...
    const task = await this.getQuery(
      'SELECT workflow_id FROM workflow_tasks WHERE id = ?',
      [taskId]
    );
    if (task) {
  ...
  private async updateWorkflowProgress(workflowId: string): Promise<void> {
    ...
    const progress = Math.round(
      (completedTasks.completed / totalTasks.total) * 100
    );
  ...
  async deleteWorkflow(workflowId: string): Promise<void> {
    ...
    await this.runQuery('DELETE FROM workflow_tasks WHERE workflow_id = ?', [
      workflowId,
    ]);

    // Delete workflow
    await this.runQuery('DELETE FROM spec_workflows WHERE id = ?', [
      workflowId,
    ]);
