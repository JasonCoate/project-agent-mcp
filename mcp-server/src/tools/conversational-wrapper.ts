...
export class ConversationalWrapper {
  ...
  async wrapToolCall(
    ...
    try {
      ...
      // Post-call summary
      const postCallMessage = this.generatePostCallMessage(toolName, result, context);
   ...
   */
  private generatePreCallMessage(toolName: string, args: any, context?: any): string {
   ...
   */
  private generatePostCallMessage(toolName: string, result: any, context?: any): string {
   ...
   */
  private generateErrorMessage(toolName: string, error: any, context?: any): string {
   ...
   */
  private updateWorkflowProgress(workflowId: string, toolName: string, result: any): void {
    try {
      const progressEntry = {
  ...
  private inferPurpose(toolName: string, args: any): string {
    const purposeMap: Record<string, (args: any) => string> = {
      ...
      'update_task_progress': (args) => `Updating task progress`,
      'create_feature_workflow': (args) => `Starting feature workflow: ${args.feature_name}`,
      'complete_workflow_task': (args) => `Marking workflow task as complete`,
      'add_workflow_task': (args) => `Adding custom workflow task`,
      'get_workflow_summary': (args) => `Generating workflow progress summary`
    };
   ...
   */
  private formatResultSummary(toolName: string, result: any): string {
    let summary = '';
   ...
   */
  private getSuggestedActions(toolName: string, error: any): string {
   ...
   */
  generateCheckpoint(phase: string, tasks: any[], workflowId?: string): string {
    const completedTasks = tasks.filter(t => t.completed);
    const totalTasks = tasks.length;
    const progress = totalTasks > 0 ? (completedTasks.length / totalTasks * 100).toFixed(1) : '0';
