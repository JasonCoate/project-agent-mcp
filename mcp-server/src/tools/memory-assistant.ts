...
export class MemoryAssistantTool {
    ...
    return [
      {
        ...
        description: "Store current session context, decisions, and analysis for future retrieval. Use this to persist important information that would normally be lost between sessions.",
        inputSchema: {
          type: "object",
          properties: {
            ...
            session_type: {
              type: "string",
              enum: ["analysis", "implementation", "debugging", "planning", "review"],
      ...
      },
      {
        ...
        description: "Retrieve relevant context and memory for the current task. Use this at the start of sessions to understand previous work and decisions.",
        inputSchema: {
          type: "object",
          properties: {
            project_id: { type: "string", description: "Project ID" },
      ...
      },
      {
        ...
        description: "Create a comprehensive knowledge snapshot of the current project state. Use this to capture complete understanding for complex projects.",
        inputSchema: {
          type: "object",
          properties: {
            project_id: { type: "string", description: "Project ID" },
      ...
      },
      {
        ...
        description: "Query the project knowledge base with natural language. Use this to find specific information without manual searching.",
        inputSchema: {
          type: "object",
          properties: {
            project_id: { type: "string", description: "Project ID" },
            ...
            knowledge_types: {
              ...
              items: {
                type: "string",
                enum: ["decisions", "architecture", "code_changes", "insights", "challenges", "patterns"]
              },
  ...
  private async retrieveRelevantContext(args: any): Promise<any> {
    const recentMemory = await this.db.getRecentMemory(args.project_id, args.max_results || 10);
    
    // Filter by session types if specified
    let filteredMemory = recentMemory;
    if (args.session_types && args.session_types.length > 0) {
      filteredMemory = recentMemory.filter(entry => 
    ...
    const relevantContext = {
      ...
      relevant_sessions: filteredMemory.map(entry => ({
        ...
        decisions: args.include_decisions ? entry.metadata.session_data?.decisions_made || [] : [],
        code_changes: args.include_code_changes ? entry.metadata.session_data?.code_changes || [] : [],
  ...
  private async queryProjectKnowledge(args: any): Promise<any> {
    ...
    const scoredResults = recentMemory.map(entry => {
      let score = 0;
