...
export class DatabaseManager {
  ...
  constructor(dbPath: string = process.env.DATABASE_PATH || './data/project-agent.db') {
  ...
  async getSpecsByProject(projectId: string): Promise<Spec[]> {
    return new Promise((resolve, reject) => {
      this.db.all('SELECT * FROM specs WHERE project_id = ? ORDER BY priority DESC, created_at DESC', [projectId], (err, rows) => {
        if (err) reject(err);
  ...
  async getTasksByProject(projectId: string): Promise<Task[]> {
    return new Promise((resolve, reject) => {
      this.db.all('SELECT * FROM tasks WHERE project_id = ? ORDER BY status, created_at DESC', [projectId], (err, rows) => {
        if (err) reject(err);
  ...
  async getRecentMemory(projectId: string, limit: number = 10): Promise<MemoryEntry[]> {
    return new Promise((resolve, reject) => {
      this.db.all('SELECT * FROM memory_log WHERE project_id = ? ORDER BY timestamp DESC LIMIT ?', [projectId, limit], (err, rows) => {
        if (err) reject(err);
        else {
